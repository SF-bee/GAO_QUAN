    ...
    for (auto i : ma) {
        for (int j = n; j >= 0; j--) {
            ...
            if (q[i].size() == 1 && j >= v[i] + v[q[i][0]]) // 只有一个附件
                dp[j] = std::max(dp[j],
                                 dp[j - v[i] - v[q[i][0]]] + v[i] * p[i] + v[q[i][0]] * p[q[i][0]]);
            if (q[i].size() == 2) {
                if (j >= v[i] + v[q[i][0]]) // 买主+A
                    dp[j] = std::max(dp[j], dp[j - v[i] - v[q[i][0]]] + v[i] * p[i] +
                                                v[q[i][0]] * p[q[i][0]]);
                if (j >= v[i] + v[q[i][1]]) // 买主+B
                    dp[j] = std::max(dp[j], dp[j - v[i] - v[q[i][1]]] + v[i] * p[i] +
                                                v[q[i][1]] * p[q[i][1]]);
                if (j >= v[i] + v[q[i][0]] + v[q[i][1]]) // 买主+AB
                    dp[j] = std::max(dp[j], dp[j - v[i] - v[q[i][0]] - v[q[i][1]]] + v[i] * p[i] +
                                                v[q[i][0]] * p[q[i][0]] + v[q[i][1]] * p[q[i][1]]);
            }
        }
    }
    std::cout << dp[n] << '\n';
